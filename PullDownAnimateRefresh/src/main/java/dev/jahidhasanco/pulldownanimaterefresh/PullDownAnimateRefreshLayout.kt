package dev.jahidhasanco.pulldownanimaterefresh
...
import dev.jahidhasanco.pulldownanimaterefresh.animations.AnimationView
import dev.jahidhasanco.pulldownanimaterefresh.animations.DefaultAnimationView
import dev.jahidhasanco.pulldownanimaterefresh.animations.LottieAnimationView
import dev.jahidhasanco.pulldownanimaterefresh.operation.RefreshCallbacks
import dev.jahidhasanco.pulldownanimaterefresh.utils.DragDistanceConverter
import kotlin.math.abs

class PullDownAnimateRefreshLayout(context: Context?, attrs: AttributeSet? = null) :
    ...
    init {
        ...
        mRefreshLayoutParams =
            MarginLayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, mRefreshViewSize)
    ...
    fun setGifAnimation(rawResource: Int) {
        if (mRefreshView is AnimationView) {
    ...
    fun setImageAsRefresh(imageResource: Int) {
        if (mRefreshView is AnimationView) {
            (mRefreshView as AnimationView).setImageResource(imageResource)
        } else {
            throw java.lang.Exception(lottieAnimViewGifMethodError)
        }
    }
    ...
    override fun onLayout(changed: Boolean, left: Int, top: Int, right: Int, bottom: Int) {
        ...
        try {
            ...
            Log.e(
                logTag,
                "error: ignored=" + ignored.toString() + " " + ignored.stackTrace.toString()
            )
        }
    ...
    private fun measureRefreshView(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val childWidthMeasureSpec: Int = if (lp.width == ViewGroup.LayoutParams.MATCH_PARENT) {
            val width =
                0.coerceAtLeast(measuredWidth - paddingLeft - paddingRight - lp.leftMargin - lp.rightMargin)
        ...
        val childHeightMeasureSpec: Int = if (lp.height == ViewGroup.LayoutParams.MATCH_PARENT) {
            val height =
                0.coerceAtLeast(measuredHeight - paddingTop - paddingBottom - lp.topMargin - lp.bottomMargin)
        ...
        when (ev.action) {
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (mIsAnimatingToStart) {
                    ...
                    Log.i(
                        logTag, "animateToStart overscrollY $overScrollY -- $mInitialMotionY"
                    )
                } else {
                ...
                if (mIsRefreshing) {
                    ...
                    Log.i(
                        logTag,
                        "moveSpinner refreshing -- " + mInitialScrollY + " -- " + (activeMoveY - mInitialMotionY)
                    )
            ...
            MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                if ((mActivePointerId == INVALID_POINTER || getMotionEventY(
                        ev,
                        mActivePointerId
                    ) == -1f)
                ) {
    ...
    private fun moveSpinner(targetOrRefreshViewOffsetY: Float) {
        ...
        if (!mIsRefreshing) {
            when (mRefreshStyle) {
                RefreshStyle.FLOAT -> {
                    convertScrollOffset = (mRefreshInitialOffset + mDragDistanceConverter.convert(
